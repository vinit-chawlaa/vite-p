...
const Hero = () => {
  ...
  return (
    <div className='px-5 lg:px-20 py-30 grid lg:grid-cols-2 grid-cols-1 gap-10 justify-items-center w-full items-center' id='home'>
       ...
       <motion.h1 variants={container(0)} initial = 'hidden' animate = 'visible' className='text-3xl lg:text-left text-center'>Hello I'm</motion.h1>
        <motion.h1 variants={container(0.2)} initial = 'hidden' animate = 'visible' className='bg-gradient-to-r lg:text-left text-center text-5xl from-purple-800 via-purple-500 to-purple-400 bg-clip-text text-transparent font-extrabold'>Vinit Chawla</motion.h1>
        <motion.h1 variants={container(0.4)} initial = 'hidden' animate = 'visible' className='text-3xl lg:text-left text-center'>
           <span ref={elref}></span>
        </motion.h1>
       </div>
        <motion.p variants={container(0.6)} initial = 'hidden' animate = 'visible' className='py-5 lg:text-left text-center'>I craft high-performance, responsive web applications that blend aesthetics with functionality.
        With a focus on clean code and scalable architecture, I help brands thrive online.</motion.p>
   ...
   <button className='px-5 py-2   text-white rounded-md cursor-pointer bg-gradient-to-r from-purple-700 via-purple-600 to-purple-800 hover:from-[#5B21B6] hover:to-[#6D28D9] transition-all duration-300'>Downlaod CV </button>
      ...
      <div className='flex gap-3'>
       ...
       <div className='py-5'>
        <motion.img  initial={{x : 100 , opacity : 0}} animate={{x : 0, opacity : 1}} transition={{duration : 1 , delay : 0.8}} src='/hero.jpeg' width={500} height={100} className='animate-[float_3s_ease-in-out_infinite] rounded-full shadow-[0_0_30px_20px_rgba(88,28,135,0.6)] transition duration-500 hover:shadow-[0_0_40px_20px_rgba(139,92,246,0.8)] border-5 border-purple-500 cursor-pointer' alt='not found'/>
       </div>
    </div>
  )
}
